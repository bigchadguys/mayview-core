...
public class MouseRatModel extends HierarchicalModel<MouseRatEntity> {
    private final ModelPart mouserat;
    private final ModelPart head;


    public MouseRatModel(ModelPart root) {
    ...
    @Override
    public void setupAnim(MouseRatEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        this.applyHeadRotation(netHeadYaw, headPitch);
        if (entity.isInWaterOrBubble()) {
            this.animate(entity.swimAnimationState, MouseRatAnimations.swim, ageInTicks, 1f);
        } else {
            this.animateWalk(MouseRatAnimations.walk, limbSwing, limbSwingAmount, 6f, 2.5f);
            this.animate(entity.idleAnimationState, MouseRatAnimations.idle, ageInTicks, 1f);
        }
    }
